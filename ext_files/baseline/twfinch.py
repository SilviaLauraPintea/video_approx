################################################################################
### TWFINCH original code
# Original code from M. Saquib Sarfraz
...
def clust_rank(mat, initial_rank=None, distance="cosine", use_tw_finch=False):
    ...
    if initial_rank is not None:
        ...
        if not pynndescent_available:
            raise MemoryError(
                "You should use pynndescent for inputs larger than {} samples.".format(
                    ANN_THRESHOLD
                )
            )
...
def cool_mean(M, u):
    ...
    umat = sp.csr_matrix(
        (np.ones(s, dtype="float32"), (np.arange(0, s), u)), shape=(s, len(un))
    )
...
def FINCH(
    ...
    if tw_finch:
        ...
        time_index = (np.arange(n_frames) + 1.0) / n_frames
    ...
    min_sim = None
    adj, orig_dist = clust_rank(
        data, initial_rank, distance=distance, use_tw_finch=tw_finch
    )
    initial_rank = None
    ...
    while exit_clust > 1:
        adj, orig_dist = clust_rank(
            mat, initial_rank, distance=distance, use_tw_finch=tw_finch
        )
    ...
    if req_clust is not None:
        if req_clust not in num_clust:
            if req_clust > num_clust[0]:
                ...
                req_c = req_numclust(
                    c[:, ind[-1]], data, req_clust, distance, use_tw_finch=tw_finch
                )
        else:
...
def start_run(args, data, labels=None):
    c, num_clust, req_c = FINCH(
        data,
        initial_rank=None,
        req_clust=args.approx_size,
        distance="cosine",
        ensure_early_exit=False,
        verbose=True,
    )
