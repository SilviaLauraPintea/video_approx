...
# Dataset class for reading "MPII_Cooking" IDT (improved dense trajectory) features using the features provided in:
...
class MP2Video:
    ...
    def read_one_data(self, idx: int):
        ...
        gt_str = np.loadtxt(
            os.path.join(gt_path, self._datalist[idx][0:-4] + ".txt"), dtype="U"
        )
        ...
        gt = np.array(
            [int(np.where(np.array(subactions) == gs)[0][0]) for gs in gt_str]
        ).astype(int)
